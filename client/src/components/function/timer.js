...
export default class Timer extends Component {
  ...
  setProgress = () => {
    const { startingMinutes, startingSeconds, minutes, seconds } = this.state;
    ...
    const progress =
      ...
      seconds === 0
        ...
        : (
            ((minutes * 60 + seconds) /
              (startingMinutes * 60 + startingSeconds)) *
            100
          ).toString();
  ...
  render() {
    const { started, minutes, seconds, progress } = this.state;

    return (
      <div className="timer-container">
        <div className="timer">
          <button className="minus-time" onClick={this.subtractMinute}>
            -
          </button>
          <span>
            {minutes}:{this.renderSeconds(seconds)}
          </span>
          <button className="add-time" onClick={this.addMinute}>
            +
          </button>
        </div>
